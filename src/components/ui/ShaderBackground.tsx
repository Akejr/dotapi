...
/**
 * ShaderBackground — "Neuro Noise", generated by the 21st.dev Shader Builder.
 ...
 *    it.
 *  - Frames already pause when the canvas scrolls out of view or the tab is
 *    hidden, which is kept.
 */
...
vec3 linearToSrgb(vec3 c) {
  ...
  // channel negative, and pow(negative, …) is NaN which mix()/step() would then
  // propagate.
...
/* Recipe from the builder. The palette is already DOT's blues, which is why this
   preset was chosen: black, brand blue, cyan, near-white. */
const UNIFORMS = {
  colors: [
    [0, 0, 0],
    [0.12156862745098039, 0.3176470588235294, 1],
    [0, 0.8980392156862745, 1],
    [0.9176470588235294, 0.9921568627450981, 1],
    [0.9176470588235294, 0.9921568627450981, 1],
    [0.9176470588235294, 0.9921568627450981, 1],
    [0.9176470588235294, 0.9921568627450981, 1],
    [0.9176470588235294, 0.9921568627450981, 1],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    /* Bail quietly rather than leaving a black rectangle over the hero. */
    ...
    /* Arrow consts rather than function declarations: TypeScript does not carry
       the non-null narrowing of `canvas` and `gl` into a hoisted function body.
       `requestRender` reads `render` lazily, and nothing runs until the call at
       the end of this effect, so the mutual reference is safe. */
    const requestRender = () => {
